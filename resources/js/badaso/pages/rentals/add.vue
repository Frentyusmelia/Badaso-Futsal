<template>
    <div>
        <template v-if="!isMaintenance">
            ...
            <vs-row
                v-if="$helper.isAllowedToModifyGeneratedCRUD('add', dataType)"
            >
                <vs-col vs-lg="12">
                    <vs-card>
                        <div slot="header">
                        ...
                        <vs-row>
                            <vs-col vs-lg="12">
                                <h3>
                                    {{
                                        $t(
                                            "crudGenerated.warning.notAllowedToAdd",
                                            {
                                                tableName:
                                                    dataType.displayNameSingular,
                                            }
                                        )
                                    }}
                                </h3>
...
export default {
    ...
    watch: {
        dataType: {
            handler(newValue) {
                let total = 0;
                for (let schedule of this.relationData.schedules) {
                    if (schedule.id == this.dataType.dataRows[3].value) {
                        console.log(this.dataType.dataRows[3].value, "value1");
                        total = schedule.price;
                        console.log(total, "price");
                    }
                }
                this.dataType.dataRows[4].value = total;
                console.log(this.dataType.dataRows[4].value, "value");
            },
            deep: true,
        },
    },
    methods: {
        submitForm() {
            ...
            // start request
            this.$openLoader();
            let scheduleid = dataRows.schedule_id;
        ...
        async getDataType() {
            ...
            try {
                ...
                console.log(this.dataType.dataRows, " datarow");
            } catch (error) {
                if (error.status == 503) {
    ...
    computed: {
        isOnline: {
            get() {
                const isOnline =
                    this.$store.getters["badaso/getGlobalState"].isOnline;
                return isOnline;
            },
        },
        base64PathName() {
